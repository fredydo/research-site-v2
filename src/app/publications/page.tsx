...
type Publication = {
  ...
  researchLine: string; type: string; doi?: string
}
type FormData = { citation: string; year: string; type: string; researchLine: string; paperUrl: string; bibtexCitation: string; doi: string }
...
const EMPTY: FormData = { citation: '', year: String(new Date().getFullYear()), type: 'JOURNAL ARTICLES', researchLine: 'PATTERN ANALYSIS AND SIGNAL PROCESSING', paperUrl: '', bibtexCitation: '', doi: '' }
...
export default function PublicationsPage() {
  ...
  const openEdit = (p: Publication) => { setEditTarget(p); setForm({ citation: p.citation, year: p.yearShort || getYear(p.year), type: p.type || 'JOURNAL ARTICLES', researchLine: p.researchLine, paperUrl: p.paperUrl || p.pdfUrl || '', bibtexCitation: p.bibtexCitation || '', doi: p.doi || '' }); setShowForm(true) }
